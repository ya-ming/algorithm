...
TEST(union_find, balanced_quick_union_uf)
{
    QuickUnionUF ob(10);

    ob.weighted_union_connections(4, 3);
    ob.weighted_union_connections(3, 8);
    ob.weighted_union_connections(6, 5);
    ob.weighted_union_connections(9, 4);
    
    ob.weighted_union_connections(2, 1);
    EXPECT_TRUE(ob.connected(8, 9));
    EXPECT_FALSE(ob.connected(5, 0));

    ob.weighted_union_connections(5, 0);
    ob.weighted_union_connections(7, 2);
    ob.weighted_union_connections(6, 1);

    EXPECT_TRUE(ob.connected(0, 2));
    EXPECT_FALSE(ob.connected(1, 9));

    ob.weighted_union_connections(7, 3);
    EXPECT_TRUE(ob.connected(1, 9));
